...
import {
  ...
  writeRawOneHopCache,
} from "../scripts/project-graph/project-evidence/task-evidence-cache.ts";
...
describe("raw one-hop Task-local cache", () => {
  ...
  it("rebinds current schedule acquisition provenance without changing semantics", () => {
    ...
    expect(rebound.schedule.parents[0]?.evidence[0]).toMatchObject({
      locator: "current-locator",
      detail: { relationDirection: "上游" },
    });
    expect(rebound.parents[0]?.scheduleEvidence[0]).toMatchObject({
      locator: "current-locator",
    });
  });
});
