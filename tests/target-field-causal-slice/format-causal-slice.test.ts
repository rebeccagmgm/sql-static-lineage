...
function branch(id: string, producerTaskId: string | null): CausalSliceArtifact["candidateUniverse"]["branches"][number] {
  return {
    ...
    consumerTaskId: "100",
    producerTaskId,
    table: producerTaskId === null ? null : {
      platform: "hive",
    ...
    readOccurrence: producerTaskId === null ? null : {
    ...
    evidenceRefs: [{ evidenceRefId: `candidate-evidence:${id}`, source: "TABLE_ARTIFACT", locator: id }],
    gapRefs: producerTaskId === null ? ["gap:unbound"] : [],
    boundaryReason: producerTaskId === null ? "producer task is unresolved" : null,
  };
}
...
function artifact(): CausalSliceArtifact {
  ...
  return {
    ...
    inputFingerprints: {
      inputPack: [{ fingerprint: "input-fingerprint", reference: "input-pack" }],
      machineFacts: [{ fingerprint: "facts-fingerprint", reference: "machine-facts" }],
      producerIndex: [{ fingerprint: "producer-fingerprint", reference: "producer-index" }],
      tableMultiHopArtifact: [{ fingerprint: "table-fingerprint", reference: "table-artifact" }],
    },
    dependencies: {
      definitions: [
        {
          ...
          proofRefs: [{ proofRefId: "proof-ref:expression", kind: "SOURCE_SPAN", refId: "expr:1" }],
        },
        {
          ...
          proofRefs: [{ proofRefId: "proof-ref:predicate", kind: "GAP", refId: "gap:predicate" }],
        },
      ],
      applications: [{
      ...
      edges: [{
        ...
        fromSubject: { subjectKind: "PHYSICAL_FIELD", physicalFieldId: SOURCE },
    ...
    candidateUniverse: {
      rootTaskId: "100",
      status: "INCOMPLETE",
      branches: [sourceBranch, unknownBranch, branch("branch:unrelated", "300")],
    ...
    traversal: {
      options: { maxDepth: 10, maxValueStates: 20, maxValuePaths: 30, maxControlStates: 40, maxControlPaths: 50 },
      roots: [{
        root: { rootTargetFieldId: ROOT, taskId: "100" },
        visitedStateKeys: [],
        activeCycleChecks: 0,
        frontiers: { VALUE: 1, EXPRESSION_CONTROL: 0, ROWSET_CONTROL: 1, WINDOW_CONTEXT: 0, RELATION_CONTEXT: 0 },
        paths: [],
        gaps: [],
        decision: {
    ...
    limits: {
      ...
      control: { maxStates: 40, maxPaths: 50, truncated: true, reasons: ["MAX_CONTROL_PATHS_REACHED"] },
    },
    assessments: [
      { assessmentId: "assessment:confirmed", pairId: "pair:confirmed", rootTargetFieldId: ROOT, candidateBranchId: sourceBranch.candidateBranchId, status: "CONFIRMED_RELATED", reasonCode: "CONTINUOUS_CONFIRMED_PATH", positiveProofIds: [positiveProof.proofId], negativeProofIds: [], gapRefs: [] },
      { assessmentId: "assessment:conditional", pairId: "pair:conditional", rootTargetFieldId: ROOT, candidateBranchId: "branch:unrelated", status: "CONDITIONAL_RELATED", reasonCode: "CONTINUOUS_PROVISIONAL_PATH", positiveProofIds: [], negativeProofIds: [], gapRefs: [] },
      { assessmentId: "assessment:unrelated", pairId: "pair:unrelated", rootTargetFieldId: ROOT, candidateBranchId: "branch:unrelated", status: "PROVEN_UNRELATED", reasonCode: "EXPLICIT_SAFE_RULES_ONLY", positiveProofIds: [], negativeProofIds: [negativeProof.proofId], gapRefs: [] },
      { assessmentId: "assessment:unknown", pairId: "pair:unknown", rootTargetFieldId: ROOT, candidateBranchId: unknownBranch.candidateBranchId, status: "UNKNOWN", reasonCode: "BRANCH_BOUNDARY_UNRESOLVED", positiveProofIds: [], negativeProofIds: [], gapRefs: ["gap:unbound"] },
    ],
    positiveProofs: [positiveProof],
    negativeProofs: [negativeProof],
    assessmentGaps: [{ gapId: "gap:unbound", rootTargetFieldId: ROOT, candidateBranchId: unknownBranch.candidateBranchId, reasonCode: "BRANCH_BOUNDARY_UNRESOLVED", evidenceRefs: ["evidence:unbound"] }],
    rerunSets: {
      minimumConfirmed: {
        kind: "MINIMUM_CONFIRMED",
        taskIds: ["200"],
        entries: [{ taskId: "200", unresolvedReason: null, triggers: [{ rootTargetFieldId: ROOT, candidateBranchId: sourceBranch.candidateBranchId, assessmentId: "assessment:confirmed", causalStatus: "CONFIRMED_RELATED", positiveProofIds: [positiveProof.proofId], negativeProofIds: [], gapRefs: [] }] }],
        unresolved: [],
      },
      conservativeSafety: {
        ...
        taskIds: ["200"],
        entries: [],
        unresolved: [{ taskId: null, unresolvedReason: "UNBOUND_READ_TASK_ID_UNRESOLVED", triggers: [{ rootTargetFieldId: ROOT, candidateBranchId: unknownBranch.candidateBranchId, assessmentId: "assessment:unknown", causalStatus: "UNKNOWN", positiveProofIds: [], negativeProofIds: [], gapRefs: ["gap:unbound"] }] }],
      },
    },
    qualityMetrics: {
    ...
    boundaries: { staticSqlOnly: true, runtimeExecution: "NOT_EVALUATED", dataCorrectness: "NOT_EVALUATED", businessAcceptance: "NOT_EVALUATED" },
...
describe("causal-slice text formatter", () => {
  it("renders all required independent artifact sections and rerun safety information", () => {
    ...
    expect(output).toContain("ASSESSMENTS_BY_ROOT_FIELD");
    expect(output).toContain("ROOT_FIELD " + ROOT);
    for (const status of ["CONFIRMED_RELATED", "CONDITIONAL_RELATED", "PROVEN_UNRELATED", "UNKNOWN"]) expect(output).toContain(status);
    expect(output).toContain("candidate=branch:source");
    expect(output).toContain("producerTask=200");
    expect(output).toContain("table=hive/warehouse/demo.source/source-id/SCHEMA_BACKED");
    expect(output).toContain("occurrence=read:100:0;relation:100:0;statement=0;path=relation:100:0");
    ...
    expect(output).toContain("UNRESOLVED_TASK_MAPPING task=- reason=UNBOUND_READ_TASK_ID_UNRESOLVED");
  });
  ...
  it("is deterministic and changes only when the supplied artifact changes", () => {
    ...
    expect(formatCausalSlice({ ...first, contentHash: "different-hash" })).not.toBe(formatCausalSlice(first));
