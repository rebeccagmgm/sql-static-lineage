...
describe("target table causal closure baseline", () => {
  ...
  it("only emits proven unrelated with complete negative evidence", () => {
    const candidate = branch({ candidateBranchId: "branch:absent" });
    const provider = { scanCount: 1, edgeCount: 0, lookup: (value: CandidateBranch) => ({ candidateBranchId: value.candidateBranchId, status: "PROVEN_ABSENT" as const, affectedTargetFields: [], outputFieldBindingIds: [], evidenceRefs: [], gapRefs: [] }) };
    ...
    expect(assessment.negativeProofRefs).toHaveLength(1);
  ...
  it("rolls multiple branches up without counting the root write", () => {
    const confirmed = canonicalAssessment({ targetWriteId: "write", candidateBranchId: "branch:p", relationStatus: "CONFIRMED_RELATED", channelAssessments: [], evidenceRefs: ["e"], gapRefs: [], negativeProofRefs: [] });
    const unknown = canonicalAssessment({ targetWriteId: "write", candidateBranchId: "branch:q", relationStatus: "UNKNOWN", channelAssessments: [{ channel: "ROW_MEMBERSHIP", status: "UNKNOWN", proofRefs: [], witnessRefs: [], gapRefs: ["g"] }], evidenceRefs: [], gapRefs: ["g"], negativeProofRefs: [] });
