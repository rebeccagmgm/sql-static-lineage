...
import { parseTargetCausalOverlayCli } from "../scripts/project-graph/target-causal-overlay/target-causal-overlay-cli.ts";
import { buildQueryIndex } from "../scripts/project-graph/query-index/query-index-builder.ts";
import { InMemoryQueryIndexStore } from "../scripts/project-graph/query-index/in-memory-query-index-store.ts";
import {
  explainIndexedTargetCausalAssessment,
  getIndexedTargetCausalOverlay,
  getIndexedTargetCausalTaskRollup,
} from "../scripts/project-graph/query-index/indexed-target-causal-overlay-query.ts";
import { runRequiredQueryIndexParity } from "../scripts/project-graph/query-index/query-index-parity.ts";
import { loadQueryIndexSource } from "../scripts/project-graph/query-index/query-index-source.ts";
import { buildTargetCausalOverlayProjection } from "../scripts/project-graph/target-causal-overlay/target-causal-overlay-projector.ts";
import {
  loadTargetCausalOverlayDirectory,
  publishTargetCausalOverlay,
} from "../scripts/project-graph/target-causal-overlay/target-causal-overlay-publication.ts";
import {
  explainTargetCausalAssessment,
  getTargetCausalOverlay,
  getTargetCausalTaskRollup,
} from "../scripts/project-graph/target-causal-overlay/target-causal-overlay-query.ts";
import {
  TARGET_CAUSAL_OVERLAY_SOURCE_CONTRACT,
  loadTargetCausalOverlaySource,
} from "../scripts/project-graph/target-causal-overlay/target-causal-overlay-source.ts";
import {
  canonicalAssessment,
...
describe("target causal overlay source and projection", () => {
  ...
  it("fails closed on causal bytes, topology hash and field hash mismatches", () => {
    ...
    const {
      contentHash: _topologyContentHash,
    ...
    writeFileSync(topologyFixture.causalPath, canonicalJson(wrongTopology), "utf8");
    expect(() =>
      loadTargetCausalOverlaySource({
        projectTopologyDirectory:
...
describe("target causal overlay publication and queries", () => {
  it("publishes atomically, reuses exact bytes and exposes bounded explanations", () => {
    ...
    expect(loadTargetCausalOverlayDirectory(first.directory).projection).toEqual(
      projection,
    );
