...
export type QueryIndexProjectionKind =
  "PROJECT_TOPOLOGY" | "FIELD_EVIDENCE" | "TARGET_CAUSAL_OVERLAY";
...
export function validateQueryIndexSourceDescriptor(
  ...
  const sortedCausal = [...causalSources].sort(compareProjectionSource);
  ...
  for (const causal of causalSources) {
    ...
    if (snapshots.has(causal.snapshotId))
      throw new Error(
        `QUERY_INDEX_CAUSAL_SOURCE_DUPLICATE:${causal.snapshotId}`,
      );
