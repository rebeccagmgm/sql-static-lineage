...
describe("asset graph write partition binding", () => {
  it("keeps same-table SQL writes separated by their Facts write observation", () => {
    const writers = taskWriters(
      projection([
        { id: "write:one", target: "dm.target" },
        { id: "write:two", target: "dm.target" },
      ]),
      evidence(
        [
          {
            write_observation_id: "write:one",
            write_statement_id: "stmt:one",
            physical_dataset: "dm.target",
          },
          {
            write_observation_id: "write:two",
            write_statement_id: "stmt:two",
            physical_dataset: "dm.target",
          },
        ],
        [
          {
            ...
            raw_sql:
              "insert overwrite table dm.target partition(p='ONE') select 1",
          },
          {
            statement_id: "stmt:two",
            raw_sql:
              "insert overwrite table dm.target partition(p='TWO') select 1",
          },
        ],
      ),
    );
  ...
  it("uses Facts before a Pack and leaves a missing write binding unknown", () => {
    const writers = taskWriters(
      projection([
        { id: "write:dynamic", target: "dm.target" },
        { id: "write:missing", target: "dm.target" },
      ]),
      evidence(
        [
          {
            write_observation_id: "write:dynamic",
            write_statement_id: "stmt:dynamic",
            physical_dataset: "dm.target",
    ...
    expect(writers[0]?.partition).toEqual([
    ...
    expect(writers[1]?.partition).toEqual([
      expect.objectContaining({
        column: "__unresolved_partition__",
        values: [],
      }),
    ]);
  });
  ...
  it("allows a Pack only for its uniquely written target", () => {
    const writers = taskWriters(
      projection([
        { id: "write:target", target: "dm.target" },
        { id: "write:other", target: "dm.other" },
      ]),
      evidence(
        [
          {
            write_observation_id: "write:target",
            physical_dataset: "dm.target",
            partition_mode: "STATIC",
            partition_assignments: [{ field: "p", status: "CONFIRMED" }],
          },
  ...
  it("fails closed when Facts NONE conflicts with an exact SQL partition", () => {
    const writers = taskWriters(
      ...
      evidence(
        [
          {
            write_observation_id: "write:none",
            write_statement_id: "stmt:none",
            physical_dataset: "dm.target",
            partition_mode: "NONE",
          },
        ],
        [
          {
            statement_id: "stmt:none",
            raw_sql:
              "insert overwrite table dm.target partition(p='03') select 1",
          },
        ],
      ),
    );
    expect(writers[0]?.partition).toEqual([
      expect.objectContaining({ values: [], partitionStatus: "UNKNOWN" }),
    ]);
  });
});
