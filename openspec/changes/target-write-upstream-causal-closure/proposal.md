...
现有 `target-field-causal-slice` 以目标字段为遍历根，在 209119 这类宽表上形成“目标字段 × 候选分支”的笛卡尔积，导致大量重复路径和 gap，运行时间、内存和结果可读性都不可接受。重跑影响分析真正需要的是目标表写入观测受哪些上游生产分支影响，同时保留字段血缘作为解释证据，并覆盖 JOIN、过滤、聚合、行存在性和重复度等非直接字段影响。
...
- 保留现有 field-lineage 作为 `FIELD_VALUE` 证据和字段级钻取入口；不修改旧 artifact、CLI、renderer 或其 contract。
- 将静态 `relationStatus` 与运行期 `rerunDecision` 分离；输出最小确定候选集和保守安全候选集，但不把静态结论冒充实际运行重跑结论。
...
- 只读消费现有 Input Pack、Plan/Machine Facts、producer index、table multi-hop artifact 和 field-lineage evidence；不替换 parser、IR、物理身份或跨 Task bridge。
