 ...
 *
 * It does not write bundles, JSONL, manifests, or Machine Facts.
...
import { tableFromDirectEvidence } from "../scripts/input/collect-one-task-input-pack.ts";
...
function findTask(dataRoot: string, taskId: string, category?: string): string {
	...
	if (matches.length !== 1) {
		throw new Error(`expected one task match for ${taskId}, got: ${matches.join(", ") || "none"}`);
	}
	return matches[0]!;
}
...
function resolutionSummary(plan: any): JsonRecord {
	...
	for (const relation of plan.relations ?? []) {
		for (const expression of [...(relation.expressions ?? []), ...(relation.measures ?? [])]) {
...
function inspectSql(sql: string, filePath: string, dataRoot: string, schema?: unknown): JsonRecord {
	const session = SqlSession.create(sql, "databricks", schema ? { schema: schema as any } : undefined);
	return {
...
function discoverPhysicalInputs(filePaths: readonly string[]): string[] {
...
function main(): void {
	...
	const task = JSON.parse(readFileSync(join(taskDir, "task.json"), "utf8")) as JsonRecord;
	...
	if (filePaths.length === 0) throw new Error(`slot ${requestedSlot} not found in ${taskDir}`);
	const tablesRootArg = arg("--tables-root");
	const fetchSzdata = process.argv.includes("--fetch-szdata");
	const tablesRoot = tablesRootArg || fetchSzdata ? resolve(tablesRootArg ?? join(dataRoot, "tables")) : undefined;
	const physicalInputs = discoverPhysicalInputs(filePaths);
	let schemaLoad = tablesRoot ? loadSchemaFromTablesRoot(tablesRoot, physicalInputs) : undefined;
	...
	if (fetchSzdata && tablesRoot && schemaLoad) {
		...
		for (const qualifiedName of requestedFetchNames) {
			...
			try {
				const evidence = tableFromDirectEvidence(qualifiedName);
				if (!evidence) {
					unavailable.push(qualifiedName);
					continue;
				}
				const result = writeTableInput(dataRoot, evidence);
				fetched.push({ qualified_name: evidence.qualifiedName, changed: result.changed, directory: result.directory });
			} catch (error) {
				failures.push({ qualified_name: qualifiedName, error: error instanceof Error ? error.message : String(error) });
			}
		}
	...
	const analysisSchema = schemaLoad && schemaLoad.loaded.length > 0 ? schemaLoad.schema : undefined;
		...
		JSON.stringify(
			{
				...
				schema_load: schemaLoad
					...
					: undefined,
				files: filePaths.map((filePath) => inspectSql(readFileSync(filePath, "utf8"), filePath, dataRoot, analysisSchema)),
			},
			null,
		2,
		),
