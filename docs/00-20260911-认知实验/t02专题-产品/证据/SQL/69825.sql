...
-- source: LOCAL_CODE
-- sqlStatus: AVAILABLE
-- scriptPath: BigData-pdata_news_n/news_dm/pdata_news_n.t02_exch_quot_h_WD_A
-- observed_at: 2026-09-05T01:06:29.141Z

-- createSql
create table if not exists T02_EXCH_QUOT_H
(
...
partitioned by (busi_date string comment '数据日期', src_id string comment '数据来源',grp_id string comment '并行分组标识')
stored as ORC;
...
WITH T02_EXCH_QUOT_H_XD_SZ AS   --当日为股权登记日的深市股票，除权参考价
(
...
left outer join 
(select * from odata_n_uip.q_equ_allot where busi_date = default.pretradedate('${data_day_str}' , 1) and record_date  = '${data_day_str}')T_TL --股权登记日当日 
...
,
T02_EXCH_QUOT_H_XD_SH AS  --当日为股权登记日的沪市股票，除权参考价
(
select   F.s_info_windcode,--股票代码
...
INSERT OVERWRITE TABLE T02_EXCH_QUOT_H partition (busi_date = '${data_day_str}' , src_id = 'WD',grp_id='01' ) 
SELECT
     B.secu_id
    ,B.src_sys_prdno
    ,A.TRADE_DT         as trd_dt
    ,CRY_CD.const_cd    as Crrc_cd
    ,S_DQ_PRECLOSE      as Pric_pre_Clos
    ,S_DQ_OPEN          as Pric_open
    ,S_DQ_HIGH          as Pric_high
    ,S_DQ_LOW           as Pric_low
    ,A.S_DQ_CLOSE         as Pric_Clos
    ,S_DQ_CHANGE        as Pric_chg
    ,S_DQ_PCTCHANGE     as pct_chg
    ,S_DQ_VOLUME        as deal_Vol
    ,S_DQ_AMOUNT * 1000 as deal_Amt--元
    ,S_DQ_ADJPRECLOSE   as Pric_adjpreclose
    ,S_DQ_ADJOPEN       as Pric_adjopen
    ,S_DQ_ADJHIGH       as Pric_adjhigh
    ,S_DQ_ADJLOW        as Pric_adjlow
    ,S_DQ_ADJCLOSE      as Pric_adjclose
    ,S_DQ_ADJFACTOR     as Pric_adjfactor
    ,S_DQ_AVGPRICE      as Pric_avgprice
    ...
    ,case when A.s_dq_tradestatus = '停牌' then P.trd_Stat_cd --如果停牌，则取上一个交易日的标识
          else
               case when (SZ.rightsissue_ratio <> 0 and SZ.rightsissue_ratio is not null and SZ.rightsissue_price <> 0 and SZ.rightsissue_price is not null)
                      or (SH.rightsissue_ratio <> 0 and SH.rightsissue_ratio is not null and SH.rightsissue_price <> 0 and SH.rightsissue_price is not null)  then '1' else  '' end  
          end  as trd_Stat_cd   -- 提供给zhangf使用的专用标识, 1表示配售股份，(非分红派息) 
    ,'' as Pric_limit
    ,'' as Pric_stopping
    ,'' as tnv_deal
    ,'' as tnv_vol_aft
    ,S_DQ_AMOUNT as tnv_val_aft
    ,case when A.s_dq_tradestatus = '停牌' then P.adj_spl_prc else nvl(SZ.prcie_after_dr, SH.prcie_after_dr) end as adj_spl_prc --当日停牌，则取上一个交易日的计算结果
    ,'' as tnv_rat
    ,'' as Pric_amp
    ,'' as Pric_after_price
    ,'' as Disc_rat
...
FROM 
(select * from odata_n_uip.w_ashareeodprices where trade_dt = '${data_day_int}' and s_info_windcode  <> '689009.SH') A  ----因689009同时在A股及CDR表均有，但A股表没有历史数据，所以取CDR表，这里过滤
...
left outer join T02_EXCH_QUOT_H_XD_SZ SZ ON A.s_info_windcode = SZ.s_info_windcode and A.trade_dt  = SZ.eqy_record_dt
left outer join T02_EXCH_QUOT_H_XD_SH SH ON A.s_info_windcode = SH.s_info_windcode and A.trade_dt  = SH.eqy_record_dt
left outer join 
(select P1.* from (select * from T02_EXCH_QUOT_H where src_id = 'WD' and grp_id = '01'  and src_sys_prdno  <> 'WD-689009.SH' )P1 
     join 
  (select max(cal_date) as cal_date_max from pdata_news_n.t02_scr_trd_cal where src_id = 'TL' and cal_date < '${data_day_int}' and trd_flag ='1' and mkt_cd = 'SZSE')P2 on P1.trd_dt = P2.cal_date_max
)P ON CONCAT('WD-' ,A.s_info_windcode) = P.src_sys_prdno
