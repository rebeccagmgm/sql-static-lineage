...
-- source: LOCAL_CODE
-- sqlStatus: AVAILABLE
-- scriptPath: BigData-GF_FDM_N/EVT/PDATA_N.T05_ACCT_CTRL_CHG_EVT_RCC398.py
-- observed_at: 2026-09-05T01:07:05.091Z
...
-----------------------------------------------------------------------------------------------------
--Group2: Source Table:[ODATA_N_RCC.H_HIS_STKACCOUNTCONTROLJOUR:证券账户控制流水历史表] 
-----------------------------------------------------------------------------------------------------
SELECT
     CONCAT('RCC398-',INIT_DATE,'-',POSITION_STR)      AS  Evt_Id                  --事件编号
    ...
    ,'ODATA_N_RCC.H_HIS_STKACCOUNTCONTROLJOUR'         AS  Real_Src_Tbl            --真实源表
    ,'scr_acct_ctrl_chg'                               AS  Evt_Type_Cd             --事件类型代码 --证券账户控制变动
    ,FUND_ACCOUNT                                      AS  Chg_Acct_Agt_Id         --变动账户协议编号
    ,IF(NVL(TRIM(FUND_ACCOUNT),'')='' OR NVL(TRIM(STOCK_ACCOUNT),'')='' OR NVL(TRIM(EXCHANGE_TYPE),'')='','',CONCAT('10101-',EXCHANGE_TYPE,'-',STOCK_ACCOUNT))
                                                       AS  Chg_Acct_Agt_Modifr     --变动账户协议修饰符
    ,STOCK_ACCOUNT                                     AS  Chg_Acct                --变动账户
    ,EXCHANGE_TYPE                                     AS  Exch_Type_Cd            --市场代码
    ,EXCHANGE_TYPE                                     AS  Src_Trd_Type_Cd         --源交易类型代码
    ,'ODATA_N_RCC.H_HIS_STKACCOUNTCONTROLJOUR'         AS  Src_Tbl                 --源表
    ,'${data_day_str}'                            AS  Busi_Date               --业务日期
FROM  ODATA_N_RCC.H_HIS_STKACCOUNTCONTROLJOUR
WHERE BUSI_DATE='${data_day_str}' 

UNION ALL

-----------------------------------------------------------------------------------------------------
--Group2.: Source Table:[ODATA_N_RCC.A_STKACCOUNTCONTROLJOUR:证券账户控制流水历史表] 
-----------------------------------------------------------------------------------------------------
SELECT
     CONCAT('RCC398-',INIT_DATE,'-',POSITION_STR)      AS  Evt_Id                  --事件编号
    ,INIT_DATE                                         AS  Trd_Date                --交易日期
    ...
    ,'${data_today_str}'                          AS  Data_Upt_Date           --数据更新日期
    ,'${data_today}'                              AS  Data_Time               --数据时间
    ,'ODATA_N_RCC.A_STKACCOUNTCONTROLJOUR'             AS  Real_Src_Tbl            --真实源表
    ,'scr_acct_ctrl_chg'                               AS  Evt_Type_Cd             --事件类型代码 --证券账户控制变动
    ,FUND_ACCOUNT                                      AS  Chg_Acct_Agt_Id         --变动账户协议编号
    ,IF(NVL(TRIM(FUND_ACCOUNT),'')='' OR NVL(TRIM(STOCK_ACCOUNT),'')='' OR NVL(TRIM(EXCHANGE_TYPE),'')='','',CONCAT('10101-',EXCHANGE_TYPE,'-',STOCK_ACCOUNT))
                                                       AS  Chg_Acct_Agt_Modifr     --变动账户协议修饰符
    ...
    ,EXCHANGE_TYPE                                     AS  Exch_Type_Cd            --市场代码
    ,EXCHANGE_TYPE                                     AS  Src_Trd_Type_Cd         --源交易类型代码
    ,'ODATA_N_RCC.A_STKACCOUNTCONTROLJOUR'             AS  Src_Tbl                 --源表
    ,'${data_day_str}'                            AS  Busi_Date               --业务日期
FROM  ODATA_N_RCC.A_STKACCOUNTCONTROLJOUR
WHERE BUSI_DATE='${data_day_str}' 
