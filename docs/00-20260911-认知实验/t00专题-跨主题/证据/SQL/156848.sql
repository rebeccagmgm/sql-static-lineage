...
-- source: LOCAL_CODE
-- sqlStatus: AVAILABLE
-- scriptPath: BigData-GF_FDM_N/CSA/PDATA_N.T00_PRD_POOL_INFO_PRD061.py
-- observed_at: 2026-09-05T01:07:03.408Z
...
AS
-----------------------------------------------------------------------------------------------------
--Group2: Source Table:[ODATA_N_PRD.P_CUSTOM_PROD_WAREHOUSE 产品池]
-----------------------------------------------------------------------------------------------------
SELECT
        ID                           AS Pool_Id1                  --池编号
       ,WAREHOUSE_NAME               AS Pool_Name1                --池名称
       ,'PRD_SCR_POOL'               AS Pool_Cate_Cd1             --池类别代码
       ,''                           AS Pool_Type_Cd1             --池类型代码
       ,NVL(DW_CD_VAL,DATA_STATUS)   AS Pool_Stat_Cd1                  --池状态代码
       ,DESCRIPTION                  AS Remark1                   --备注
       ...
       ,WAREHOUSE_TYPE               AS Pool_Create_Type_Cd1       --池生成类型代码
       ,CREATED_TYPE                 AS Pool_Create_Method_Cd1     --池生成方式代码
...
LEFT JOIN (
         ...
         WHERE TGT_TAB_NAME = 'T00_PRD_POOL_INFO'
            AND TGT_TAB_FLD  = 'Pool_Stat_Cd'
            AND SRC_TAB_NAME = 'CUSTOM_PROD_WAREHOUSE'
            AND SRC_FLD_NAME = 'DATA_STATUS'
            AND SRC_SYS_NAME='PRD')B
   ON      A.DATA_STATUS =B.SRC_CD_VAL                  --DATA_STATUS 转码
