...
-- source: LOCAL_CODE
-- sqlStatus: AVAILABLE
-- scriptPath: BigData-GF_FDM_N/CSA/PDATA_N.T00_PRD_POOL_COMPNT_INFO_PRD062.py
-- observed_at: 2026-09-05T01:07:03.413Z
...
AS
-----------------------------------------------------------------------------------------------------
---Group2: Source Table:[ODATA_N_PRD.P_CUSTOM_WHS_PROD_REL 入池产品列表]   
-----------------------------------------------------------------------------------------------------
SELECT
     PRODWHS_ID                        AS Pool_Id                 --池编号
    ,'PRD_SCR_POOL'                    AS Pool_Cate_Cd            --池类别代码
    ,''                                AS Pool_Compnt_Type_Cd          --池成分类型代码
    ,PROD_CODE                         AS Compnt_Id               --成分
    ,''                                AS Port                    --端口
    ,DATA_SOURCE                       AS Src                     --来源
    ,IF(length(WAREHOUSING_DATE)=10,from_unixtime(unix_timestamp(WAREHOUSING_DATE ,'yyyy-MM-dd') ,'yyyy-MM-dd HH:mm:ss'),WAREHOUSING_DATE)
                                       AS In_Pool_Date            --入池日期
    ,''                                AS In_Oper                 --入池人
    ,TAG_INFO                          AS In_Pool_Rsn             --入池原因
    ,''
    ...
    ,FIELDS                            AS Pdef_Attr_Info            --自定义属性信息
...
FROM(SELECT 
           ...
           ,A.TAG_INFO
           ,B.FIELDS
           ,ROW_NUMBER()OVER(PARTITION BY A.PRODWHS_ID,A.PROD_CODE ORDER BY A.WAREHOUSING_DATE DESC ) RN
       FROM (SELECT * FROM  ${src_table} WHERE BUSI_DATE = '${data_day_str}')A
       LEFT JOIN( SELECT * FROM ODATA_N_PRD.P_CUSTOM_FIELD WHERE BUSI_DATE = '${data_day_str}') B 
              ON A.PRODWHS_ID =B.PRODWHS_ID 
