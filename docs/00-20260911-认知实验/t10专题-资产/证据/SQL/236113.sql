...
-- source: LOCAL_CODE
-- sqlStatus: AVAILABLE
-- scriptPath: BigData-GF_FDM_N/AST/PDATA_N.T10_AST_EMP_RELA_H_FAM001.py
-- observed_at: 2026-09-05T01:07:29.171Z

-- createSql
CREATE TABLE IF NOT EXISTS T10_AST_EMP_RELA_H(
         Ast_Id                     STRING COMMENT '资产编号'
        ,Ast_Emp_Rela_Type_Cd       STRING COMMENT '资产员工关系类型代码'
...
DROP TABLE IF EXISTS ${db_temp}.T10_AST_EMP_RELA_H_TEMP_FAM001;
    CREATE TABLE IF NOT EXISTS ${db_temp}.T10_AST_EMP_RELA_H_TEMP_FAM001
    AS
    -----------------------------------------------------------------------------------------------------
    ...
    SELECT
        ...
        ,'01'                                    AS  Ast_Emp_Rela_Type_Cd1   --资产员工关系类型代码              01 --固定资产责任人
        ,IF(NVL(TRIM(B.EMP_ID),'')='',A.ASSET_PERSON_ERP_CODE,B.EMP_ID)
                                                 AS  Emp_Id1                 --员工编号
    ...
    LEFT JOIN(SELECT DISTINCT USER_ID,EMP_ID FROM PDATA_N.T04_USER_EMP_RELA_H WHERE SRC_TBL='ODATA_N_OAS.P_GF_USER' AND DEL_FLAG='0')B
            ON A.ASSET_RESPONSIBLE_PERSON =B.USER_ID
    ;

DROP TABLE IF EXISTS ${db_temp}.T10_AST_EMP_RELA_H_MID_FAM001;
    CREATE TABLE ${db_temp}.T10_AST_EMP_RELA_H_MID_FAM001
    AS
    SELECT
           A.*,B.*
           ,CASE WHEN A.Ast_Id IS NULL     AND B.Ast_Id1 IS NOT NULL THEN 'I'           --当天不存在历史的数据为新增
                 ...
                 (   COALESCE(A.Emp_Id                  ,'')   <> COALESCE(B.Emp_Id1                 ,'')
             ) THEN 'U'                                                        --当天和历史均存在,除主键外如有字段变更为变更
                 ELSE 'S' END AS DATA_TYPE                                         --其他为无变更
    FROM   (  SELECT
                *
              FROM  T10_AST_EMP_RELA_H
              WHERE STRT_DATE <='${data_day_str}'
                AND END_DATE  > '${data_day_str}'
                AND SRC_TBL IN ('ODATA_N_FAM.W_ASSET_ACCOUNT')  )A   --历史(昨日)开链数据
    FULL OUTER JOIN ${db_temp}.T10_AST_EMP_RELA_H_TEMP_FAM001 B               --当天的数据
    ON   A.Ast_Id= B.Ast_Id1
    AND  A.Ast_Emp_Rela_Type_Cd= B.Ast_Emp_Rela_Type_Cd1
    ;

-- querySql
INSERT OVERWRITE TABLE T10_AST_EMP_RELA_H PARTITION(SRC_TBL)
     --剔除掉源表当日新增和当日闭链的数据
    SELECT 
         DISTINCT
         Ast_Id                                --资产编号
        ,Ast_Emp_Rela_Type_Cd                  --资产员工关系类型代码
        ,Strt_Date                             --开始日期
        ,Emp_Id                                --员工编号
        ,End_Date                              --结束日期
        ,Data_Src_Cd                           --数据来源代码
...
set hive.merge.mapfiles = true ;
    ...
    SELECT   
        ...
        ,Ast_Emp_Rela_Type_Cd1          AS  Ast_Emp_Rela_Type_Cd    --资产员工关系类型代码
    ...
    FROM ${db_temp}.T10_AST_EMP_RELA_H_MID_FAM001 WHERE DATA_TYPE IN ('I','U')     --新增-I,变更-U
    UNION ALL
     --历史开链有变动和不存在当天的数据闭链
    SELECT
         Ast_Id                                                     --资产编号
    ...
    FROM ${db_temp}.T10_AST_EMP_RELA_H_MID_FAM001 WHERE DATA_TYPE IN ('D','U')     --变更-U,删除-D
    UNION ALL
     --无变动的数据插回目标表
    SELECT
         Ast_Id                                                     --资产编号
        ...
        ,Real_Src_Tbl                                               --真实源表
        ,Src_Tbl                                                    --源表
    FROM ${db_temp}.T10_AST_EMP_RELA_H_MID_FAM001 WHERE DATA_TYPE ='S'         --无变更-S
    ;
