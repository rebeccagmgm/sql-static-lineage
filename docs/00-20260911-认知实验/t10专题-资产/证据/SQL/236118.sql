...
-- source: LOCAL_CODE
-- sqlStatus: AVAILABLE
-- scriptPath: BigData-GF_FDM_N/AST/PDATA_N.T10_AST_INR_ORG_RELA_H_FAM001.py
-- observed_at: 2026-09-05T01:07:29.203Z

-- createSql
CREATE TABLE IF NOT EXISTS T10_AST_INR_ORG_RELA_H(
         Ast_Id                     STRING COMMENT '资产编号'
        ,Ast_Inr_Org_Rela_Type_Cd   STRING COMMENT '资产内部机构关系类型代码'
...
DROP TABLE IF EXISTS ${db_temp}.T10_AST_INR_ORG_RELA_H_TEMP_FAM001;
    CREATE TABLE IF NOT EXISTS ${db_temp}.T10_AST_INR_ORG_RELA_H_TEMP_FAM001
    AS
    -----------------------------------------------------------------------------------------------------
    ...
    SELECT
        ...
        ,'01'                                    AS  Ast_Inr_Org_Rela_Type_Cd1--资产内部机构关系类型代码            01 --费用承担部门公司段
        ,IF(NVL(TRIM(COST_BEAR_DEPT_CODE),'')='','',CONCAT('ERP-CO-',SUBSTR(COST_BEAR_DEPT_CODE,1,5)))
                                                 AS  Inr_Org_Id1             --内部机构编号    
        ,'5'                                     AS  Inr_Org_Cate_Cd1        --内部机构类别代码
    ...
    SELECT
         ASSET_TAG_NO                            AS  Ast_Id1                 --资产编号                     
        ,'02'                                     AS  Ast_Inr_Org_Rela_Type_Cd1--资产内部机构关系类型代码            02 --费用承担部门部门段
        ,IF(NVL(TRIM(COST_BEAR_DEPT_CODE),'')='','',CONCAT('ERP-DEPT-',SUBSTR(COST_BEAR_DEPT_CODE,6)))
                                                 AS  Inr_Org_Id1             --内部机构编号    
        ,'5'                                     AS  Inr_Org_Cate_Cd1        --内部机构类别代码
        ,'${data_src_cd}'                   AS  Data_Src_Cd1            --数据来源代码
        ,'${src_table}'                     AS  Src_Tbl1                --源表
    ...
    SELECT
         ASSET_TAG_NO                            AS  Ast_Id1                 --资产编号                     
        ,'03'                                    AS  Ast_Inr_Org_Rela_Type_Cd1--资产内部机构关系类型代码           03 --使用部门
        ,ASSET_USE_DEPT                          AS  Inr_Org_Id1             --内部机构编号    
        ,'4'                                     AS  Inr_Org_Cate_Cd1        --内部机构类别代码
        ,'${data_src_cd}'                   AS  Data_Src_Cd1            --数据来源代码
        ,'${src_table}'                     AS  Src_Tbl1                --源表
        ,'${filename}'                      AS  Task_Name1              --任务名
        ,'${src_table}'                     AS  Real_Src_Tbl1           --真实源表
...
DROP TABLE IF EXISTS ${db_temp}.T10_AST_INR_ORG_RELA_H_MID_FAM001;
    CREATE TABLE ${db_temp}.T10_AST_INR_ORG_RELA_H_MID_FAM001
    AS
    SELECT
           A.*,B.*
           ,CASE WHEN A.Ast_Id IS NULL     AND B.Ast_Id1 IS NOT NULL THEN 'I'           --当天不存在历史的数据为新增
                 ...
                 (   COALESCE(A.Inr_Org_Id              ,'')   <> COALESCE(B.Inr_Org_Id1             ,'')
                  OR COALESCE(A.Inr_Org_Cate_Cd         ,'')   <> COALESCE(B.Inr_Org_Cate_Cd1        ,'')
             ) THEN 'U'                                                        --当天和历史均存在,除主键外如有字段变更为变更
                 ELSE 'S' END AS DATA_TYPE                                         --其他为无变更
    FROM   (  SELECT
                *
              FROM  T10_AST_INR_ORG_RELA_H
              WHERE STRT_DATE <='${data_day_str}'
                AND END_DATE  > '${data_day_str}'
                AND SRC_TBL IN ('ODATA_N_FAM.W_ASSET_ACCOUNT')  )A   --历史(昨日)开链数据
    FULL OUTER JOIN ${db_temp}.T10_AST_INR_ORG_RELA_H_TEMP_FAM001 B               --当天的数据
    ON   A.Ast_Id= B.Ast_Id1
    AND  A.Ast_Inr_Org_Rela_Type_Cd= B.Ast_Inr_Org_Rela_Type_Cd1
    ;

-- querySql
INSERT OVERWRITE TABLE T10_AST_INR_ORG_RELA_H PARTITION(SRC_TBL)
     --剔除掉源表当日新增和当日闭链的数据
    SELECT 
         DISTINCT
         Ast_Id                                --资产编号
        ,Ast_Inr_Org_Rela_Type_Cd              --资产内部机构关系类型代码
        ,Strt_Date                             --开始日期
        ,Inr_Org_Id                            --内部机构编号
...
set hive.merge.mapfiles = true ;
    ...
    SELECT   
        ...
        ,Ast_Inr_Org_Rela_Type_Cd1       AS  Ast_Inr_Org_Rela_Type_Cd--资产内部机构关系类型代码
    ...
    FROM ${db_temp}.T10_AST_INR_ORG_RELA_H_MID_FAM001 WHERE DATA_TYPE IN ('I','U')     --新增-I,变更-U
    UNION ALL
     --历史开链有变动和不存在当天的数据闭链
    SELECT
         Ast_Id                                                     --资产编号
    ...
    FROM ${db_temp}.T10_AST_INR_ORG_RELA_H_MID_FAM001 WHERE DATA_TYPE IN ('D','U')     --变更-U,删除-D
    UNION ALL
     --无变动的数据插回目标表
    SELECT
         Ast_Id                                                     --资产编号
        ...
        ,Real_Src_Tbl                                               --真实源表
        ,Src_Tbl                                                    --源表
    FROM ${db_temp}.T10_AST_INR_ORG_RELA_H_MID_FAM001 WHERE DATA_TYPE ='S'         --无变更-S
    ;
