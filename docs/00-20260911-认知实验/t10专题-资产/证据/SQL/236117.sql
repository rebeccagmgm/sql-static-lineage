...
-- source: LOCAL_CODE
-- sqlStatus: AVAILABLE
-- scriptPath: BigData-GF_FDM_N/AST/PDATA_N.T10_AST_CLAS_H_FAM001.py
-- observed_at: 2026-09-05T01:07:29.197Z

-- createSql
CREATE TABLE IF NOT EXISTS T10_AST_CLAS_H(
         Ast_Id                     STRING COMMENT '资产编号'
        ,Ast_Clas_Type_Cd           STRING COMMENT '资产分类类型代码'
        ,Strt_Date                  STRING COMMENT '开始日期'
        ,Ast_Clas_Cd                STRING COMMENT '资产分类代码'
...
DROP TABLE IF EXISTS ${db_temp}.T10_AST_CLAS_H_TEMP_FAM001;
    CREATE TABLE IF NOT EXISTS ${db_temp}.T10_AST_CLAS_H_TEMP_FAM001
    AS
    -----------------------------------------------------------------------------------------------------
    ...
    SELECT
        ...
        ,'01'                                    AS  Ast_Clas_Type_Cd1       --资产分类类型代码                 '01'  --固定资产资产分类
        ,ASSET_PARENT                            AS  Ast_Clas_Cd1            --资产分类代码                   
    ...
    SELECT
         ASSET_TAG_NO                            AS  Ast_Id1                 --资产编号                     
        ,'02'                                    AS  Ast_Clas_Type_Cd1       --资产分类类型代码                '02'  --固定资产资产分类细分
        ,ASSET_SON                               AS  Ast_Clas_Cd1            --资产分类代码                   
        ,'${data_src_cd}'                   AS  Data_Src_Cd1            --数据来源代码
        ,'${src_table}'                     AS  Src_Tbl1                --源表
        ,'${filename}'                      AS  Task_Name1              --任务名
        ,'${src_table}'                     AS  Real_Src_Tbl1           --真实源表
    ...
    SELECT
         ASSET_TAG_NO                            AS  Ast_Id1                 --资产编号                     
        ,'03'                                    AS  Ast_Clas_Type_Cd1       --资产分类类型代码                '03'  --固定资产性
        ,ASSET_NATURE                            AS  Ast_Clas_Cd1            --资产分类代码                   
        ,'${data_src_cd}'                   AS  Data_Src_Cd1            --数据来源代码
        ,'${src_table}'                     AS  Src_Tbl1                --源表
        ,'${filename}'                      AS  Task_Name1              --任务名
        ,'${src_table}'                     AS  Real_Src_Tbl1           --真实源表
...
DROP TABLE IF EXISTS ${db_temp}.T10_AST_CLAS_H_MID_FAM001;
    CREATE TABLE ${db_temp}.T10_AST_CLAS_H_MID_FAM001
    AS
    SELECT
           A.*,B.*
           ,CASE WHEN A.Ast_Id IS NULL     AND B.Ast_Id1 IS NOT NULL THEN 'I'           --当天不存在历史的数据为新增
                 ...
                 (   COALESCE(A.Ast_Clas_Cd             ,'')   <> COALESCE(B.Ast_Clas_Cd1            ,'')
    ...
    FULL OUTER JOIN ${db_temp}.T10_AST_CLAS_H_TEMP_FAM001 B               --当天的数据
    ON   A.Ast_Id= B.Ast_Id1
    AND  A.Ast_Clas_Type_Cd= B.Ast_Clas_Type_Cd1
    ;

-- querySql
INSERT OVERWRITE TABLE T10_AST_CLAS_H PARTITION(SRC_TBL)
     --剔除掉源表当日新增和当日闭链的数据
    SELECT 
         DISTINCT
         Ast_Id                                --资产编号
        ,Ast_Clas_Type_Cd                      --资产分类类型代码
        ,Strt_Date                             --开始日期
        ,Ast_Clas_Cd                           --资产分类代码
        ,End_Date                              --结束日期
        ,Data_Src_Cd                           --数据来源代码
...
set hive.merge.mapfiles = true ;
    ...
    SELECT   
        ...
        ,Ast_Clas_Type_Cd1              AS  Ast_Clas_Type_Cd        --资产分类类型代码
        ,'${data_day_str}'         AS  Strt_Date               --开始日期
        ,Ast_Clas_Cd1                   AS  Ast_Clas_Cd             --资产分类代码
        ,'2099-12-31'                   AS  End_Date                --结束日期
    ...
    FROM ${db_temp}.T10_AST_CLAS_H_MID_FAM001 WHERE DATA_TYPE IN ('I','U')     --新增-I,变更-U
    UNION ALL
     --历史开链有变动和不存在当天的数据闭链
    SELECT
         Ast_Id                                                     --资产编号
    ...
    FROM ${db_temp}.T10_AST_CLAS_H_MID_FAM001 WHERE DATA_TYPE IN ('D','U')     --变更-U,删除-D
    UNION ALL
     --无变动的数据插回目标表
    SELECT
         Ast_Id                                                     --资产编号
        ...
        ,Real_Src_Tbl                                               --真实源表
        ,Src_Tbl                                                    --源表
    FROM ${db_temp}.T10_AST_CLAS_H_MID_FAM001 WHERE DATA_TYPE ='S'         --无变更-S
    ;
