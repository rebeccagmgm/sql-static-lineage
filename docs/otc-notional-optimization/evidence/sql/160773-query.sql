...
select 
	...
	t1.busi_mon
from 
(select * from dm_index_n.index_grp2_std_ast_bef_covt_mth_apd
...
union all 
select grp_id3 grp_id1,grp_id1 grp_id2,grp_id2 grp_id3,'tag074443736' tag_id,sum(cast(index_val as double)) index_val,busi_mon
...
union all 
select grp_id1,grp_id2,grp_id3,tag_id,sum(cast(index_val as double)) index_val,busi_mon 
from 	dm_index_n.index_grp3_CompScal_OtcDeri_Cs_MthApd 
where busi_mon BETWEEN substr(default.date2datekey(ADD_MONTHS('${yyyy-MM}-01',-6)),1,6)  and '${yyyyMM}'
group by 1,2,3,4,busi_mon
)

SELECT grp1.grp_id AS grp_id1, grp2.grp_id AS grp_id2, grp3.grp_id AS grp_id3, index.index_val, from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') data_time, 'wxfuyangxun' modify_operator, '2999-12-31 00:00:00' modify_time, '1' status, 'ind2024062615516761' index_id, index.busi_mon busi_mon, tag_id FROM ( 
...
from 
	...
	join 
	(select * from dm_index_n.grp_def where status = '1' and grp_type_code in ('COMPANY','CORP_CUST', 'CSTD_MOT_PRD', 'INDV_CUST', 'OFFLINE_CUST', 'OTC_COUNTERPARTY'))b1 
