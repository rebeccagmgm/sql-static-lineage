...
任务 `149961` 将对手方权限业务类型从 `odata_n_tit.d_ref_ctpty_auth_busi` 同字段覆盖到 `pdata_nds.d_ref_ctpty_auth_busi`；任务 `150768` 将 OTC 合同属性名和值从 `odata_n_tit.d_trd_otc_contr_props` 同字段覆盖到 `pdata_nds.trd_otc_contr_props`。两项均写动态业务日期分区，当前投影各有一个外部读取和一个最终写入、无材料缺口；SQL 未包含额外关联或聚合。因此这里能够确认的是接口字段搬运形态，不能据此证明任务实际运行、数据到达或属性值的业务口径。[任务 149961](../../../../sql-static-lineage-data/task-projections/tasks/149961/versions/86c166048a995b4c4d96d9296f7d044c0361a646515e0a4d526eaa2bf69b9a06.evidence-v3.json)、[任务 150768](../../../../sql-static-lineage-data/task-projections/tasks/150768/versions/037a8422f3ccbdb577b6475c755c35840b81a18e5fc358c09f7f7585b14923cc.evidence-v3.json)
...
- **材料范围。** `149961`、`150768` 已生成可用数据 IO 投影并补入上文接口说明。`213177/213178/213180/213182/213185` 于 2026-09-08 核验为正常状态的手工周期任务，并非冻结任务；它们不进入自动生产血缘覆盖范围，也不再作为本页材料缺口。
- **批次分区与业务日期。** `141369/160827/163712/210338/210339/222317/243712` 以及资金页 `213442/236414/236415` 在源 `_p` 表的 `busi_date` 上使用 `h15`、`h1930` 等日内批次值，目标分区则可使用实际业务日期。当前投影分别保存源读取谓词与目标写入分区；不能仅因字段同名就将批次值判为日期错误，也不能由静态 SQL 证明某批次实际有数。
- **同表多生产者按分区区分。** 历史 `hiveTask` 与 `hiveTask2.0`、PB 和日终版本可以写同一物理表，这是正常形态。生产者接续按物理表及可证明的分区范围匹配：已知分区不同即互斥；只有分区证据缺失或范围确实重叠时才保留多生产者歧义。静态血缘仍不自动选择所谓“最新任务”。
