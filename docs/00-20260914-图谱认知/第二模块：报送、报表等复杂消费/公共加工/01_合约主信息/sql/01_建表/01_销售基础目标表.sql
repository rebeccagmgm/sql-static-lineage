-- 【目标结构】四个分支共用的90个非分区字段；SELECT按位置写入，各分支11_输出字段.sql必须同序。
-- 【分区组织】Busi_Date=加工日；Grp_Id=01期权/02普通互换/03金仕达/04指定FAST TRS。
-- 【变更边界】CREATE TABLE IF NOT EXISTS不会修改已经存在的表；本地DDL不能证明线上结构已一致。
-- 【类型边界】源DDL业务列均为string；计算表达式的类型/隐式转换沿用原SQL，不在注释补充时调整。
-- 采用86840/86841/220650共同定义。86842原DDL Futr_Type_Cd 与此处Futr_Type不一致，部署前需核对真实表。
