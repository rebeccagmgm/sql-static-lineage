with nom_prin as(
    ...
    from(
        ...
        and coalesce(Early_Term_Date, End_Pric_Date) > '${yyyy-MM-dd}' -- 当前有效合约
    ) t0
    -- 动态名义本金
    inner join PDATA_N.T98_OTC_DERI_COMP_SALE_ADTNL_DET t1
    on t0.agt_id = t1.agt_id
    and t1.busi_date = '${yyyy-MM-dd}'
    and t1.End_Pric_Date > '${yyyy-MM-dd}' -- 当前有效合约
)

SELECT grp.grp_id AS grp_id, index.index_val, from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') data_time, 'wxliangjiaqi' modify_operator, '2999-12-31 00:00:00' modify_time, '1' status, 'ind2024091124468232' index_id, index.busi_date busi_date, tag_id FROM (

-- 1.tagdim102729 场外衍生品业务类型
SELECT
    '8888' as grp_val,
    'COMPANY' as grp_type_code,
    SUM(t.Dyna_Nom_Prin) AS index_val,
    tg.tag_id,
...
select
    '8888' as grp_val
    ,'COMPANY' as grp_type_code
    ,sum(t0.Dyna_Nom_Prin) as index_val
    ,tag.tag_id
    ,'${yyyy-MM-dd}' as busi_date
from nom_prin t0
left join(
...
FROM (
    select Book_Agt_Id,
        Ori_Crrd_Dyna_Nom_Prin * CASE WHEN Crrc_Cd = 'CNY' THEN 1 
                                 WHEN Lcrrc_Crrc_Cd = 'CNY' THEN Lcrrc_Ori_Bgng_Rate 
                                 WHEN Sett_Crrc_Cd = 'CNY' THEN Sett_Bgng_Rate end as Dyna_Nom_Prin
