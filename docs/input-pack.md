...
insufficient evidence. `sparkIndex` also applies the controlled convention
that a confirmed partitioned target with `busi_date` as a partition field may
use `${YYYY-MM-DD}` when that field's write expression is unresolved; any
other partition field must still be uniquely proven, otherwise the whole map
is omitted. Other task categories retain their existing resolver behavior.
