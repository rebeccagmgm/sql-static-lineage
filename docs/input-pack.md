...
`task.json.partition` keeps the compact field-to-value configuration shape, for example `{ "busi_date": "${YYYY-MM-DD}" }`. A key is emitted only when it is a target table partition field and its value comes from scheduler configuration or an explicit target write. Source-table predicates and source-extraction SQL are not target partition mappings. Runtime values such as `${YYYY-MM-DD}` remain templates rather than rendered business dates.
