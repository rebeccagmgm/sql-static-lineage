...
export function loadTargetCausalOverlayDirectory(
  ...
  const manifest = parseJson<TargetCausalOverlayProjectionManifestV1>(
    readBounded(join(directory, TARGET_CAUSAL_OVERLAY_MANIFEST_FILE), maxFileBytes),
    "MANIFEST",
  );
  ...
  if (
    ...
    projection.snapshot.runtimeRerunDecision !== manifest.runtimeRerunDecision ||
...
function assertDirectoryMatches(
  ...
  for (const fileName of PUBLISHED_FILES) {
    ...
    if (!existsSync(path) || readFileSync(path, "utf8") !== expected.get(fileName))
