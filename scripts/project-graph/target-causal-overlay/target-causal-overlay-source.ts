...
import { validateCausalClosure } from "../../reconcile/consumer/target-table-upstream-causal-closure/proof-validator.ts";
import {
...
export function loadTargetCausalOverlaySource(
  ...
  const project = loadProjectTopologyDirectory(
    input.projectTopologyDirectory,
    { maxFileBytes },
  );
...
export function validateTargetTableCausalArtifact(
  ...
  if (canonicalizeTargetTableArtifact(body).contentHash !== artifact.contentHash)
    throw new Error("TARGET_CAUSAL_OVERLAY_CAUSAL_HASH_INVALID");
  const targetWriteId = artifact.targetWrite.identity.targetWriteId;
  if (
    artifact.targetWrite.identity.taskId !== artifact.candidateUniverse.rootTaskId
  )
  ...
  if (missingGap)
    throw new Error(`TARGET_CAUSAL_OVERLAY_REFERENCED_GAP_MISSING:${missingGap}`);
  ...
  for (const taskId of [
    ...
    if (!rollupTaskIds.has(taskId))
      throw new Error(`TARGET_CAUSAL_OVERLAY_TASK_SET_MEMBER_MISSING:${taskId}`);
}
...
function assertSourcesAlign(input: {
  ...
  if (
    ...
    fieldSnapshot.projectSource.manifestContentHash !== project.manifest.contentHash ||
    fieldSnapshot.projectSource.manifestSha256 !== input.projectSource.manifestSha256 ||
    fieldSnapshot.projectSource.snapshotSha256 !== input.projectSource.snapshotSha256 ||
    fieldSnapshot.projectSource.nodesSha256 !== input.projectSource.nodesSha256 ||
  ...
  if (
    ...
    fieldSnapshot.selection.writeObservationId !== identity.writeObservationId ||
...
function fieldSourceRef(
  ...
  return {
    snapshotId: source.manifest.snapshotId,
    manifestContentHash: source.manifest.contentHash,
    manifestSha256: fileSha(join(source.directory, FIELD_EVIDENCE_MANIFEST_FILE)),
    snapshotSha256: source.manifest.files.snapshot.sha256,
    nodesSha256: source.manifest.files.nodes.sha256,
    edgesSha256: source.manifest.files.edges.sha256,
...
function parseCausalArtifact(bytes: Buffer): TargetTableCausalClosureArtifact {
  try {
    return JSON.parse(bytes.toString("utf8")) as TargetTableCausalClosureArtifact;
  } catch {
