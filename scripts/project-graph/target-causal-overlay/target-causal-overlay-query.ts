...
> {
  ...
  const channelsByAssessment = groupedNodes(
    projection.nodes.filter(({ nodeType }) => nodeType === "CHANNEL_ASSESSMENT"),
    "assessmentId",
  );
...
> {
  ...
  const taskMatches = projection.nodes.filter(
    (node) =>
      node.nodeType === "TASK_REF" && node.properties.taskId === taskId,
  );
...
> {
  ...
  const selection: CausalSelection = {
    targetWrite: full.targetWrite.filter(({ nodeId }) => allowedNodes.has(nodeId)),
    assessments: full.assessments.filter(({ nodeId }) => allowedNodes.has(nodeId)),
...
function selectAssessmentContext(
  ...
  const taskIds = new Set(
    branches.flatMap((branch) =>
      [branch.properties.consumerTaskId, branch.properties.producerTaskId].filter(
  ...
  const tasks = projection.nodes.filter(
    (node) =>
      node.nodeType === "TASK_REF" &&
      taskIds.has(text(node.properties.taskId)),
  );
