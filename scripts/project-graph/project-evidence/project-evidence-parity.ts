...
import type { MultiHopReconciliationResult } from "../../reconcile/consumer/multi-hop/reconcile-multi-hop.ts";
...
export function compareProjectRootTraversal(
  ...
  for (const section of SECTIONS) {
    const expectedValue = normalized(expected[section]);
    const actualValue = normalized(actual[section]);
    const expectedCanonical = canonicalJson(expectedValue);
    const actualCanonical = canonicalJson(actualValue);
