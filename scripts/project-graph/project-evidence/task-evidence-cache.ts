...
import type { PrefetchedScheduleEvidence } from "../../pipeline/lineage-all.ts";
import {
...
export function rawOneHopProducerEvidenceHash(
  ...
  const scheduleParentTaskIds = [
    ...new Set(result.schedule.parents.map((parent) => parent.taskId)),
  ].sort(compareText);
...
function resolvedDirectReadTables(
  ...
  for (const read of result.currentTask.directReads) {
    const platform = requiredTablePart(read.table.platform);
    const dataSource = requiredTablePart(read.table.dataSource);
    const qualifiedName = requiredTablePart(read.table.qualifiedName);
    if (!platform || !dataSource || !qualifiedName) continue;
    const table = { platform, dataSource, qualifiedName };
    byKey.set(
