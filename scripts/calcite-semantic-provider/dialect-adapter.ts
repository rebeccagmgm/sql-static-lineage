...
const QUOTABLE = new Set(["CONDITION", "DEFAULT", "OPERATOR"]);
...
export function adaptHiveCompatSql(sql: string): DialectTransformResult {
  ...
  for (let index = 0; index < tokens.length; index++) {
    ...
    if (token.kind !== "WORD" || !QUOTABLE.has(token.text.toUpperCase()))
      continue;
    const previous = tokens[index - 1];
    const next = tokens[index + 1];
    const quote =
      previous?.text === "." ||
      previous?.text.toUpperCase() === "AS" ||
      next?.text === ".";
    if (quote)
      replacements.push({
        start: token.start,
  ...
  const transforms = replacements
    ...
    .map((replacement) => {
      ...
      return {
        ...
        afterSpan: {
          start: afterStart,
          end: afterStart + replacement.text.length,
        },
        reason:
          "Calcite Lex.JAVA reserves this identifier; backtick quoting preserves identifier identity and relation semantics.",
      };
    });
  return {
