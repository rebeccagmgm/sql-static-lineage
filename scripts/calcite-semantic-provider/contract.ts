...
export type MappingStatus = "EXACT" | "UNMAPPABLE";
...
export type DependencyKind = "VALUE_INPUT" | "EXPRESSION_SELECTOR" | "FILTER_PREDICATE" | "JOIN_MATCH" | "JOIN_CARDINALITY" | "GROUP_KEY" | "AGGREGATE_INPUT" | "SET_MEMBERSHIP" | "WINDOW_VALUE" | "WINDOW_PARTITION" | "WINDOW_ORDER" | "WINDOW_FRAME" | "ORDER_SELECTION" | "RELATION_EXISTENCE";
export type ImpactKind = "FIELD_VALUE" | "EXPRESSION_CONTROL" | "ROW_MEMBERSHIP" | "MULTIPLICITY" | "GROUPING" | "SET_MEMBERSHIP" | "WINDOW_EFFECT" | "ORDER_SELECTION" | "RELATION_EXISTENCE";
...
export interface CandidateTaskSemanticFacts {
  ...
  readonly relations: readonly { readonly relationId: string; readonly kind: OperatorKind; readonly inputRelationIds: readonly string[]; readonly outputFieldIds: readonly string[]; readonly qualifiedTableName?: string; readonly providerOrdinal?: number }[];
  ...
  readonly operators: readonly { readonly operatorId: string; readonly kind: OperatorKind; readonly relationId: string; readonly inputRelationIds: readonly string[]; readonly joinType?: "INNER" | "LEFT" | "RIGHT" | "FULL" | "SEMI" | "ANTI" | "CROSS" }[];
...
const MAPPING = new Set<MappingStatus>(["EXACT", "UNMAPPABLE"]);
...
const DEPENDENCY = new Set<DependencyKind>(["VALUE_INPUT", "EXPRESSION_SELECTOR", "FILTER_PREDICATE", "JOIN_MATCH", "JOIN_CARDINALITY", "GROUP_KEY", "AGGREGATE_INPUT", "SET_MEMBERSHIP", "WINDOW_VALUE", "WINDOW_PARTITION", "WINDOW_ORDER", "WINDOW_FRAME", "ORDER_SELECTION", "RELATION_EXISTENCE"]);
const IMPACT = new Set<ImpactKind>(["FIELD_VALUE", "EXPRESSION_CONTROL", "ROW_MEMBERSHIP", "MULTIPLICITY", "GROUPING", "SET_MEMBERSHIP", "WINDOW_EFFECT", "ORDER_SELECTION", "RELATION_EXISTENCE"]);
...
function validateRelation(value: unknown,index:number,issues:string[]):void { if(!isRecord(value)){issues.push(`relations[${index}] must be object`);return;} exactKeys(value,["relationId","kind","inputRelationIds","outputFieldIds","qualifiedTableName","providerOrdinal"],`relations[${index}]`,issues,true); stableId(value.relationId,`relations[${index}].relationId`,issues); enumValue(value.kind,OPERATOR,`relations[${index}].kind`,issues); ids(value.inputRelationIds,`relations[${index}].inputRelationIds`,issues); ids(value.outputFieldIds,`relations[${index}].outputFieldIds`,issues); if(value.providerOrdinal!==undefined)integer(value.providerOrdinal,`relations[${index}].providerOrdinal`,issues); }
...
function validateOperator(value:unknown,index:number,issues:string[]):void { if(!isRecord(value)){issues.push(`operators[${index}] must be object`);return;} exactKeys(value,["operatorId","kind","relationId","inputRelationIds","joinType"],`operators[${index}]`,issues,true); stableId(value.operatorId,`operators[${index}].operatorId`,issues); enumValue(value.kind,OPERATOR,`operators[${index}].kind`,issues); stableId(value.relationId,`operators[${index}].relationId`,issues); ids(value.inputRelationIds,`operators[${index}].inputRelationIds`,issues); }
