...
function bounded<T>(values: readonly T[], key?: (value: T) => string): { items: T[]; total: number } {
  const ordered = key ? sorted(values, key) : [...values];
  return { items: ordered.slice(0, DETAIL_LIMIT), total: ordered.length };
}

function subject(subject: { readonly subjectKind: string; readonly physicalFieldId?: string; readonly relationOccurrenceId?: string }): string {
...
function table(branch: CausalSliceArtifact["candidateUniverse"]["branches"][number]): string {
...
function occurrence(branch: CausalSliceArtifact["candidateUniverse"]["branches"][number]): string {
...
function proofRefs(ids: readonly string[], proofs: ReadonlyMap<string, string>): string {
  return list(ids.map((id) => `${id}${proofs.has(id) ? ` {${proofs.get(id)}}` : ""}`));
}

function assessmentLine(
  assessment: CausalSliceArtifact["assessments"][number],
  branches: ReadonlyMap<string, CausalSliceArtifact["candidateUniverse"]["branches"][number]>,
  positiveProofs: ReadonlyMap<string, CausalSliceArtifact["positiveProofs"][number]>,
  negativeProofs: ReadonlyMap<string, CausalSliceArtifact["negativeProofs"][number]>,
  ...
  const lines = [
    ...
    `      negativeProofRefs=${proofRefs(assessment.negativeProofIds, new Map([...negativeProofs].map(([id, proof]) => [id, `${proof.reasonCode};evidence=${list(proof.evidenceRefs)};source=${value(proof.sourceNegativeProofId)};obligations=${proof.checkedObligations.slice(0, DETAIL_LIMIT).map((item) => `${item.kind}:${list(item.evidenceRefs)}`).join("|")}${proof.checkedObligations.length > DETAIL_LIMIT ? ` ... omitted=${proof.checkedObligations.length - DETAIL_LIMIT}` : ""}`])))} `,
...
function formatAssessments(artifact: CausalSliceArtifact): string[] {
  const branches = new Map(artifact.candidateUniverse.branches.map((branch) => [branch.candidateBranchId, branch]));
  const positiveProofs = new Map(artifact.positiveProofs.map((proof) => [proof.proofId, proof]));
  const negativeProofs = new Map(artifact.negativeProofs.map((proof) => [proof.proofId, proof]));
  const gaps = new Map(artifact.assessmentGaps.map((gap) => [gap.gapId, gap]));
  const allRoots = uniqueSorted([
    ...artifact.request.rootFields,
    ...artifact.assessments.map((assessment) => assessment.rootTargetFieldId),
  ]);
  const roots = allRoots.slice(0, DETAIL_LIMIT);
  ...
  const lines = [
    "ASSESSMENTS_BY_ROOT_FIELD",
    `  ROOT_FIELDS ${detailCount(allRoots.length, roots.length)}`,
  ...
  for (const root of roots) {
    lines.push(`  ROOT_FIELD ${root}`);
    for (const status of ASSESSMENT_STATUSES) {
      const assessments = sorted(
        artifact.assessments.filter((assessment) => assessment.rootTargetFieldId === root && assessment.status === status),
        (assessment) => `${assessment.candidateBranchId}|${assessment.assessmentId}`,
      );
      const showing = Math.min(assessments.length, remainingByStatus.get(status) ?? 0);
      lines.push(`    ${status} (${assessments.length}) ${detailCount(assessments.length, showing)}`);
      if (assessments.length === 0) lines.push("      - none");
      for (const assessment of assessments.slice(0, showing))
        lines.push(...assessmentLine(assessment, branches, positiveProofs, negativeProofs, gaps));
      remainingByStatus.set(status, (remainingByStatus.get(status) ?? 0) - showing);
    }
    const traversalRoot = artifact.traversal.roots.find((item) => item.root.rootTargetFieldId === root);
    const traversalGaps = sorted(
      artifact.traversal.gaps.filter((gap) => gap.rootTargetFieldId === root),
      (gap) => gap.gapId,
    );
    lines.push(`    traversalDecision value=${value(traversalRoot?.decision.valuePathCertainty)} control=${value(traversalRoot?.decision.controlPathCertainty)} valueClosed=${value(traversalRoot?.decision.valueClosed)} controlClosed=${value(traversalRoot?.decision.controlClosed)}`);
    lines.push(`    traversalGapRefs value=${list(traversalRoot?.decision.valueGapIds ?? [])} control=${list(traversalRoot?.decision.controlGapIds ?? [])}`);
    const showingTraversalGaps = Math.min(traversalGaps.length, remainingTraversalGaps);
    lines.push(`    traversalGaps (${traversalGaps.length}) ${detailCount(traversalGaps.length, showingTraversalGaps)}`);
    if (traversalGaps.length === 0) lines.push("      - none");
    for (const gap of traversalGaps.slice(0, showingTraversalGaps)) lines.push(`      - ${gap.gapId} task=${gap.taskId} frontier=${gap.frontierKind} reason=${gap.reasonCode} subject=${gap.subject ? subject(gap.subject) : "-"} occurrence=${value(gap.readOccurrenceId)} evidence=${list(gap.evidenceRefs)} blocksNegativeProof=${gap.blocksNegativeProof}`);
    remainingTraversalGaps -= showingTraversalGaps;
  }
  if (allRoots.length > roots.length) lines.push(`  ROOT_FIELDS_OMITTED ${allRoots.length - roots.length}`);
  return lines;
}

function formatDependencies(artifact: CausalSliceArtifact): string[] {
  const definitionSet = bounded(artifact.dependencies.definitions, (item) => item.dependencyId);
  const applicationSet = bounded(artifact.dependencies.applications, (item) => item.applicationId);
  ...
  for (const definition of artifact.dependencies.definitions) supportCounts.set(definition.supportStatus, (supportCounts.get(definition.supportStatus) ?? 0) + 1);
  ...
  for (const definition of definitions) lines.push(`    - ${definition.dependencyId} operator=${definition.operatorKind}/${definition.operatorVariant} role=${definition.operatorRole} subject=${subject(definition.subject)} effect=${definition.effectKind} localEdge=${definition.localEdgeKind} support=${definition.supportStatus} proofRefs=${list(definition.proofRefs.map((ref) => ref.proofRefId))}`);
  lines.push("  APPLICATIONS");
  if (applications.length === 0) lines.push("    - none");
  for (const application of applications) lines.push(`    - ${application.applicationId} root=${application.rootTargetFieldId} dependency=${application.dependencyId} rootDependence=${application.rootDependenceKind} certainty=${application.pathCertainty} proofRefs=${list(application.proofRefs.map((ref) => ref.proofRefId))}`);
  lines.push("  EDGES");
  if (edges.length === 0) lines.push("    - none");
  for (const edge of edges) lines.push(`    - ${edge.edgeId} ${subject(edge.fromSubject)} -> ${subject(edge.toSubject)} dependency=${edge.dependencyId} rootDependence=${edge.rootDependenceKind} localEdge=${edge.localEdgeKind} certainty=${edge.pathCertainty} proofRefs=${list(edge.proofRefs.map((ref) => ref.proofRefId))}`);
  lines.push(`  OMITTED definitions=${definitionSet.total - definitions.length} applications=${applicationSet.total - applications.length} edges=${edgeSet.total - edges.length} gaps=${gapSet.total - gapSet.items.length}`);
  return lines;
}
...
function formatRerunSet(
  ...
  const entrySet = bounded(rerunSet.entries, (entry) => `${entry.taskId ?? ""}|${entry.unresolvedReason ?? ""}`);
  const entries = entrySet.items;
  for (const entry of entries) {
    const triggerSet = bounded(entry.triggers, (trigger) => `${trigger.rootTargetFieldId}|${trigger.candidateBranchId}|${trigger.assessmentId}`);
    const triggers = triggerSet.items;
    lines.push(`  TASK_MAPPING task=${value(entry.taskId)} unresolvedReason=${value(entry.unresolvedReason)} triggers=${triggerSet.total} ${detailCount(triggerSet.total, triggers.length)}`);
    for (const trigger of triggers) lines.push(`    - root=${trigger.rootTargetFieldId} candidate=${trigger.candidateBranchId} assessment=${trigger.assessmentId} status=${trigger.causalStatus} positiveProofRefs=${list(trigger.positiveProofIds)} negativeProofRefs=${list(trigger.negativeProofIds)} gapRefs=${list(trigger.gapRefs)}`);
  }
  const unresolvedSet = bounded(rerunSet.unresolved, (entry) => `${entry.unresolvedReason ?? ""}|${entry.triggers.map((trigger) => trigger.assessmentId).join("|")}`);
  const unresolved = unresolvedSet.items;
  if (unresolved.length === 0) lines.push("  UNRESOLVED_TASK_MAPPINGS - none");
  for (const entry of unresolved) {
    const triggerSet = bounded(entry.triggers, (item) => `${item.rootTargetFieldId}|${item.candidateBranchId}|${item.assessmentId}`);
    lines.push(`  UNRESOLVED_TASK_MAPPING task=${value(entry.taskId)} reason=${value(entry.unresolvedReason)} triggers=${triggerSet.total} ${detailCount(triggerSet.total, triggerSet.items.length)}`);
    for (const trigger of triggerSet.items) lines.push(`    - root=${trigger.rootTargetFieldId} candidate=${trigger.candidateBranchId} assessment=${trigger.assessmentId} status=${trigger.causalStatus} positiveProofRefs=${list(trigger.positiveProofIds)} negativeProofRefs=${list(trigger.negativeProofIds)} gapRefs=${list(trigger.gapRefs)}`);
  }
  lines.push(`  OMITTED entries=${entrySet.total - entries.length} unresolved=${unresolvedSet.total - unresolved.length}`);
  return lines;
}
...
export function formatCausalSlice(artifact: CausalSliceArtifact): string {
  ...
  const branchSet = bounded(artifact.candidateUniverse.branches, (item) => item.candidateBranchId);
  lines.push(`  branchDetailLimit=${DETAIL_LIMIT}`);
  for (const branch of branchSet.items) lines.push(`  - candidate=${branch.candidateBranchId} kind=${branch.branchKind} producerTask=${value(branch.producerTaskId)} consumerTask=${value(branch.consumerTaskId)} rootTask=${value(branch.rootTaskId)} table=${table(branch)} occurrence=${occurrence(branch)} boundary=${value(branch.boundaryReason)}`);
  lines.push(`  branchesDetail ${detailCount(branchSet.total, branchSet.items.length)}`);
  ...
  lines.push(
    ...
    ...formatRerunSet("MINIMUM_CONFIRMED_RERUN_SET", artifact.rerunSets.minimumConfirmed),
    ...formatRerunSet("CONSERVATIVE_SAFETY_RERUN_SET", artifact.rerunSets.conservativeSafety),
  );
  return `${lines.join("\n")}\n`;
}
